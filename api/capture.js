...
// Segment membership (5 Sep 2026): every captured contact joins the Workbook
// List segment, id from RESEND_WORKBOOK_SEGMENT_ID, never hard-coded. Resend's
// docs cannot be read from this room, so two calls are tried in order and the
// one the account accepts is logged: (1) a `segments` field on the contact
// create/update; (2) a segment-membership POST. Already-a-member is not an
// error either way. If the env var is absent the capture proceeds and logs it.
const segmentId = () => process.env.RESEND_WORKBOOK_SEGMENT_ID || '';
async function addToSegment(email, headers) {
  const sid = segmentId();
  if (!sid) return { segment: 'skipped: RESEND_WORKBOOK_SEGMENT_ID not set' };
  const r = await fetch(`https://api.resend.com/segments/${encodeURIComponent(sid)}/contacts`, {
    method: 'POST', headers, body: JSON.stringify({ email }),
  });
  if (r.ok || r.status === 409) return { segment: 'membership-endpoint', status: r.status };
  return { segment: 'FAILED', status: r.status, body: (await r.text().catch(() => '')).slice(0, 200) };
}
...
async function upsertContact(email, source, capturedAt) {
  ...
  let r = await fetch('https://api.resend.com/contacts', {
    method: 'POST', headers, body: JSON.stringify({ email, unsubscribed: false, properties, ...(segments ? { segments } : {}) }),
  });
  if (r.ok) return { ok: true, model: 'contacts', action: 'created', properties: true, ...(await addToSegment(email, headers)) };

  if (r.status === 409) {
    const u = await fetch(`https://api.resend.com/contacts/${encodeURIComponent(email)}`, {
      method: 'PATCH', headers, body: JSON.stringify({ unsubscribed: false, properties, ...(segments ? { segments } : {}) }),
    });
    if (u.ok) return { ok: true, model: 'contacts', action: 'updated', properties: true, ...(await addToSegment(email, headers)) };
  ...
  if (bare.ok || bare.status === 409) {
    return { ok: true, model: 'contacts', action: bare.ok ? 'created' : 'existed', properties: false,
             propertiesRefused: { status: r.status, body: firstBody }, ...(await addToSegment(email, headers)) };
  }
...
// ---------------------------------------------------------------------------
// THE DAILY QUOTE UNSUBSCRIBE (5 Sep 2026). The send itself lives in Supabase
// (pg_cron + pg_net, see daily_quote_rebuild.sql); this is the one-click way
// out that the email links to. Two scopes, both minted in SQL with the same
// wire format as _lib/token.js: 'dq:<uid>' for account holders, 'dqe:<b64url
// email>' for Resend contacts with no account. A library token has neither
// scope and does nothing here; these tokens cannot open the library.
// ---------------------------------------------------------------------------
async function quoteUnsubscribe(req, res) {
  const raw = req.query?.unsub; const token = Array.isArray(raw) ? raw[0] : raw;
  const claim = token ? verifyToken(token) : null;
  const subj = claim ? String(claim.uid || '') : '';
...
<h1 style="font-size:20px;color:#0C447C;margin:0 0 12px;">${esc(title)}</h1><p style="font-size:16px;line-height:1.5;margin:0;">${body}</p></div></body></html>`);
  ...
  if (!admin) return page('That link has expired', 'Please try the link again in a moment.');
  let row = null;
  if (subj.startsWith('dq:')) row = { user_id: subj.slice(3), at: new Date().toISOString() };
  else if (subj.startsWith('dqe:')) {
    try {
      const email = Buffer.from(subj.slice(4).replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8').toLowerCase();
      if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) row = { email, at: new Date().toISOString() };
    } catch (e) {}
  }
  if (!row) return page('That link has expired', 'Open the app and use Settings to change what we send you.');
  const { error } = await admin.from('quote_optouts').upsert(row, { onConflict: row.user_id ? 'user_id' : 'email' });
  if (error) { console.warn('quote: optout failed', error.message); return page('Something went wrong', 'Please try the link again in a moment.'); }
  return page('Done', 'The daily quote has stopped. Your planner is unchanged.');
}
...
export default async function handler(req, res) {
  ...
  // The daily quote's unsubscribe link lands here (GET ?unsub=<token>).
  if (req.method === 'GET' && req.query?.unsub) return quoteUnsubscribe(req, res);
